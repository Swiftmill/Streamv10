...
import { writeJsonFile, readJsonFile } from "./utils/fileStore.js";
import { z } from "zod";
...
app.post("/api/auth/logout", requireAuth, csrfProtection, async (req, res) => {
...
app.post("/api/auth/reset", csrfProtection, async (req, res) => {
...
app.post("/api/movies", requireAuth, requireRole("admin"), csrfProtection, async (req, res) => {
...
app.put("/api/movies/:id", requireAuth, requireRole("admin"), csrfProtection, async (req, res) => {
...
app.delete("/api/movies/:id", requireAuth, requireRole("admin"), csrfProtection, async (req, res) => {
...
app.post("/api/series", requireAuth, requireRole("admin"), csrfProtection, async (req, res) => {
  const sanitized = sanitizePayload(req.body, ["description", "synopsis"]);
...
app.put("/api/series/:slug", requireAuth, requireRole("admin"), csrfProtection, async (req, res) => {
...
app.post("/api/series/:slug/episodes", requireAuth, requireRole("admin"), csrfProtection, async (req, res) => {
  const series = await getSeries(req.params.slug);
  if (!series) {
    return res.status(404).json({ error: "Series not found" });
  }
  ...
  const record = await upsertSeries({ ...series, episodes: [parseResult.data] }, req.user.username);
  await appendAudit({
    timestamp: dayjs().toISOString(),
    user: req.user.username,
...
app.delete("/api/series/:slug", requireAuth, requireRole("admin"), csrfProtection, async (req, res) => {
...
app.post("/api/categories", requireAuth, requireRole("admin"), csrfProtection, async (req, res) => {
...
app.put("/api/categories/:id", requireAuth, requireRole("admin"), csrfProtection, async (req, res) => {
...
app.delete("/api/categories/:id", requireAuth, requireRole("admin"), csrfProtection, async (req, res) => {
...
app.post("/api/history", requireAuth, csrfProtection, async (req, res) => {
...
app.post("/api/play", requireAuth, csrfProtection, async (req, res) => {
...
app.get("/api/admin/users", requireAuth, requireRole("admin"), adminLimiter, async (_req, res) => {
  const users = await listUsers();
  res.json({ users });
});

app.post("/api/admin/users", requireAuth, requireRole("admin"), adminLimiter, csrfProtection, async (req, res) => {
...
app.post("/api/admin/users/:username/reset", requireAuth, requireRole("admin"), adminLimiter, csrfProtection, async (req, res) => {
...
app.post("/api/admin/users/:username/deactivate", requireAuth, requireRole("admin"), adminLimiter, csrfProtection, async (req, res) => {
