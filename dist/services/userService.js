...
async function loadUsers() {
  const adminUsers = await readJsonFile(ADMIN_USERS_FILE, USER_COLLECTION_SCHEMA, []);
  const standardUsers = await readJsonFile(STANDARD_USERS_FILE, USER_COLLECTION_SCHEMA, []);
...
export async function createUser(user) {
  ...
  const record = USER_SCHEMA.parse({
    ...user,
    passwordHash,
    createdAt: now,
    updatedAt: now,
...
export async function updatePasswordWithToken(username, token, newPassword) {
  const users = await loadUsers();
  const target = users.find((user) => user.username === username);
...
export async function recordHistoryEntry(username, entry) {
...
export async function readHistory(username) {
  await mkdir(HISTORY_ROOT, { recursive: true });
  const historyFile = join(HISTORY_ROOT, `${username}.json`);
  return readJsonFile(historyFile, HISTORY_COLLECTION_SCHEMA, []);
}

export async function upsertViewToken(userId) {
  const token = uuid();
...
export async function saveUsersSnapshot(users) {
  await saveUsers(users);
}
